...
**Current: A8 — `pallet-epoch`**. A7 is complete; next up is epoch/phase clock, proposal registry/cohorts, and the decision engine.
...
| A7 | `pallet-welfare` — bounded counters, snapshots, `MetricSpec` registry, pillar pipeline (`C_onchain`/`C_attested`), gate-breach flags, settlement score | 05 §4, §6–§7 | A1, A5 | ✅ | I-16; MetricSpec registry, bounded snapshots, C split, deterministic gate flags, settlement score, try-state checks, benchmark stub, and reference-model welfare vectors are in place |
