...
**Current: M1 — `crates/futarchy-primitives`**. M0 repository bootstrap is complete: the Rust workspace/toolchain and CI skeleton exist, implementation roots are present, and V-1 platform pins were re-verified on 2026-07-13. The `docs/architecture/` write guard has been removed (2026-07-13, user decision — see Decision log / Session log): the spec is editable, changed deliberately per rule R-1.
...
| M1 | `crates/futarchy-primitives` — `no_std` shared SCALE types + kernel `K` constants, `INTEGRATION_CONTRACT_VERSION = 2` (bump applied 2026-07-13 in doc 02, see Decision log) | 02 §2; 13 §2; 01 §5.2 | M0 | 🔨 | Initial crate/types/constants landed without `frame` deps; SCALE derive dependency wiring remains to finish once registry access is available or dependencies are vendored |
