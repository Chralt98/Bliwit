...
/// `pallet-oracle` gates `adjudicate`. The authority matrix MUST say so: a call
/// classified `Public` is admitted for every origin and, being non-privileged,
/// is also carried by the proxy/multisig wrapper set — so the defence-in-depth
/// layer 06 §3.3/§3.4 requires was silently disabled for precisely the call
/// SQ-294 identifies as under-constrained. The pallet-level `EnsureOrigin` is
/// what binds today, which is why this is a hardening, not a live escalation.
#[test]
fn registry_resolve_challenge_carries_its_real_oracle_resolution_authority() {
    ...
    for call in registry_calls.iter() {
        // Same matrix row as the oracle's terminal call: OracleResolution only.
        for origin in all_class_origins() {
