...
| Tooling suites | `python3 -m unittest discover -s <dir>` over `tools/deploy/tests`, `tools/reference-model/tests`, `tools/release/tests`, `tools/env/tests` (the env suite needs `pyyaml==6.0.2` + `websockets==15.0.1`); `python3 tools/env/validate-environments.py` |
...
| `tools/release/` | B8 done | Release-artifact publication tooling (02 §11; 15 §5): reproducible-recipe runtime build, booted-node metadata extraction with `:code`↔wasm binding, the 02 critical-surface manifest + deterministic chainHead fixture recorder, content-addressed assembly with readiness report, the `bleavit.env-evidence.v1` contract (produced by `tools/env/run-evidence.py`, B7), `environments.json` live-env inventory — see `tools/release/README.md`; a real tag release fails closed until B2/A8/A11/SQ-101 land their surface and the env suites un-gate (03 B9-topology, 07 SQ-101/B4) |
...
| `runtime/` | B1a + B4 residual + B6 | `runtime/bleavit-runtime` is the real Cumulus parachain runtime (`construct_runtime!`, `impl_runtime_apis!`, `BaseCallFilter = SafetyFilter`, genesis presets with the 08 §2.1 VIT allocation/vesting). B6 wired `ExecutionGuard` (index 62) with the full upgrade path e2e; the A8/A11 follow-up wired `Epoch` live at index 61 (live epoch clock everywhere) and B5 landed generated weights/PoV budgets + weight-regression CI. B4 residual wires `InflowCaps` (index 63), Params-backed DOT/USDC `TraderRates` in the XcmConfig, the XCM-health chain (`HealthTrackingRouter` wrap → welfare `XcmTraffic` → `MetricInputs` X emission; R deliberately unbound, SQ-150), the treasury custody sync (`PotFunding`: `fund_budget_line` moves MAIN→pot USDC atomically), and the ready `PhaseInflowCaps` adapter; the XCM transactor remains blocked on SQ-101. Open follow-up: implement `FutarchyApi` from `runtime-api/` in `impl_runtime_apis!` (B2) |
