...
| Tooling suites | `python3 -m unittest discover -s <dir>` over `tools/deploy/tests`, `tools/reference-model/tests`, `tools/release/tests`, `tools/phase-gates/tests`, `tools/env/tests`, `tools/ci/tests` (the env suite needs `pyyaml==6.0.2` + `websockets==15.0.1`); `python3 tools/env/validate-environments.py` |
