...
| `runtime/` | B1–B12 + B16 done; B15 closeout | `runtime/bleavit-runtime` is the real Cumulus parachain runtime (`construct_runtime!`, `impl_runtime_apis!`, `BaseCallFilter = SafetyFilter`, genesis presets with the 08 §2.1 VIT allocation/vesting). `Epoch`, `ExecutionGuard`, and `InflowCaps` occupy frozen indices 61, 62, and 63. Existing B1–B12 wiring includes the real epoch/guard path, six-track governance, live Params consumers, treasury/XCM posture, generated weights and the 11-method contract-v9 `FutarchyApi`. B16 adds compile-time bootstrap/Phase-4 and paired recovery profiles, the exact one-shot Phase-3→4 META bridge, permissionless bounded recovery qualification, mandatory inherent-only scheduling/application, Abort restoration and frozen recovery eligibility. B15's primary registers exactly the finite `TotalEscrowed` backfill; the paired zero-SDK-MBM recovery restarts that read-only scan under lockdown and unlocks only with the exact terminal mirror/version commit. Additional MBMs remain prohibited without their own exhaustive cutpoint repair. |
...
| `runtime-api/` | B2 done | `futarchy-runtime-api`: the `sp_api::decl_runtime_apis!` declaration of the frozen 11-method `FutarchyApi` (02 §3) over the §4 view types in `futarchy-primitives`. B2 implemented all 11 in the runtime (`runtime/bleavit-runtime/src/views.rs` + `impl_runtime_apis!`) and landed the 02 amendment batch as **integration contract v4** (the contract has since moved to **v9**; `futarchy_primitives::INTEGRATION_CONTRACT_VERSION = 9` and 02 §13 are the authorities, so the next bump is v10); B13 added the separate monitoring-only `TelemetryApi` trait (`runtime-api/src/telemetry.rs` + `runtime/bleavit-runtime/src/telemetry.rs`) — explicitly outside the 02 contract, owned by 12 §6.3, consumed only by the ops exporters |
