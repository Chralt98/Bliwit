...
| `tools/release/` | B8 done + B16 in progress | Release-artifact publication tooling (02 §11; 15 §5): reproducible runtime-profile builds, booted-node metadata extraction with `:code`↔wasm binding, the 02 critical-surface manifest + deterministic chainHead fixture recorder, content-addressed assembly with readiness report, the `bleavit.env-evidence.v1` contract (produced by `tools/env/run-evidence.py`, B7), `environments.json` live-env inventory — see `tools/release/README.md`. B16 adds the machine-readable bootstrap/Phase-4 primary+recovery profile matrix: a release selects only the primary profile and automatically builds, boots and binds its same-commit zero-MBM terminal-recovery pair at exactly the next spec version. A real tag release still fails closed on missing per-release B7 evidence and the manifest's unresolved adoption blockers. |
...
| `runtime/` | B1–B12 done + B16 in progress | `runtime/bleavit-runtime` is the real Cumulus parachain runtime (`construct_runtime!`, `impl_runtime_apis!`, `BaseCallFilter = SafetyFilter`, genesis presets with the 08 §2.1 VIT allocation/vesting). `Epoch`, `ExecutionGuard`, and `InflowCaps` occupy frozen indices 61, 62, and 63. Existing B1–B12 wiring includes the real epoch/guard path, six-track governance, live Params consumers, treasury/XCM posture, generated weights and the 11-method contract-v8 `FutarchyApi`. B16 adds compile-time bootstrap/Phase-4 and paired recovery profiles, the exact one-shot Phase-3→4 META bridge, permissionless bounded recovery qualification, mandatory inherent-only terminal scheduling/application, Abort restoration, frozen recovery eligibility, zero-MBM lockdown and generated PoV coverage. Ordinary MBMs remain prohibited until a release supplies an exhaustive cutpoint repair. |
