...
> **⚠️ SUPERSEDED (2026-07-12).** This document is retained as a historical reference only. The authoritative architecture is the component document set in [`docs/architecture/`](docs/architecture/README.md), which reorganizes this specification and applies every fix mandated by [`DESIGN_REVIEW.md`](DESIGN_REVIEW.md) v2.0 per the decisions in [`docs/architecture/00-decision-record.md`](docs/architecture/00-decision-record.md). The §30 parent-document patch has been accepted in full and folded into [`docs/architecture/02-integration-contract.md`](docs/architecture/02-integration-contract.md). Do not implement from this file.

**Status:** Superseded draft (was: Draft for engineering review and audit scoping. Companion to `BACKEND-PLAN.md`; §30 contained the exact patch to the parent).
