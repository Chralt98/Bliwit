...
6. **Parameter-coupling rule (normative):** a decision changing `epoch.slots`, `mkt.obs_interval`, `dec.window`, `epoch.length`, `pol.budget_epoch`, any `pol.b.{param,trs,code,meta}`, `pol.b_gate` or `pol.b_baseline` MUST carry re-derivations of items 1–5 in its committed artifact; the classifier tags these keys with a `RederiveBudgets` obligation. **That obligation is declared, not yet enforced**: no screening check for it exists in the runtime today, so for the POL keys the operative control is the paired **CODE** proposal [08](08-treasury-and-economics.md) §4.1 requires — a re-derivation carried only in an artifact cannot move compile-time floors. Wiring the check is tracked as a code row. The **seven** POL keys are triggers, for two distinct reasons. Six of them — `pol.budget_epoch`, the four `pol.b.*` class keys and `pol.b_gate` — are triggers because [08](08-treasury-and-economics.md) §4.1's per-class NAV floors are **frozen constants derived from them**: lowering `pol.budget_epoch` or raising a `pol.b` raises the true floor above the frozen literal, so without re-derivation the loud §4.2 arming gate would pass a class below its real minimum-viable NAV. **`pol.b_baseline` is the seventh (SQ-305 resolution, 2026-07-21)**, on the narrower ground that item 5 above expressly carries the 17,329 USDC Baseline commitment among the values item 6 requires re-deriving. Its inclusion is *not* a claim that it moves the four class floors — [08](08-treasury-and-economics.md) §4.3 deliberately keeps the Baseline book outside the §4.1 floor arithmetic and outside `pol.budget_epoch` — but raising it does raise a real standing obligation against NAV on its own dedicated `POL_BASELINE` line, and item 6 promises re-derivation of item 5, not of §4.1 alone. Omitting it left a value that item 5 derives with no trigger obliging anyone to re-derive it; the omission was undocumented rather than reasoned. Books-per-proposal (6) and `MaxLiveProposals` (32) are not keys — changing them is a CODE change that reopens this document.
