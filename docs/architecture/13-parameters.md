...
Normative language: RFC 2119. Blocks at 6 s (14,400/day); NUM 6 decimals, GOV 12 decimals; `Fixed` = `FixedU64` (1e9 scale) at API boundaries.
...
| `dec.v_min` per class | Balance | NUM | 100k / 250k / 600k / 1.2M — **floors; effective value = max(floor, 2·InCapPrize), [08](08-treasury-and-economics.md) §5.3** | ×0.1 | ×10 (floors only; the 2·P term is K) | ×2 | 2 | META (vf) | [05](05-welfare-and-decision-engine.md), [08](08-treasury-and-economics.md) |
| `gate.p_max` (S, C) | Fixed | prob | 0.05 | — | **0.10 K ceiling** | 0.01 | 4 | META+values | [05](05-welfare-and-decision-engine.md) |
| `gate.v_min` (gate-book contest floor, per book) | Balance | NUM | **0.1 × `dec.v_min`(class)** | ×0.05 | ×0.5 | ×2 | 2 | META | [05](05-welfare-and-decision-engine.md) §5.2 |
...
| `prop.bond` per class | Balance | NUM | 1k / 5k + 0.5%·Ask / 25k / 50k | ×0.1 | ×10 | ×2 | 2 | META | [06](06-governance-and-guardians.md) |
...
| `pol.b` decision, per branch, per class | Balance | NUM | 10k / 25k / 60k / 100k — **floors of the Ask-scaled schedule** | budget-capped | — | 25% | 1 | TREASURY | [04](04-markets-and-pricing.md), [08](08-treasury-and-economics.md) |
| `pol.b_gate` | Balance | NUM | 7,500 | — | — | 25% | 1 | TREASURY | [04](04-markets-and-pricing.md) |
| `pol.b_baseline` | Balance | NUM | **25,000** *(sim-gated — [VERIFY via Phase-0/3 calibration])*; funded from `POL_BASELINE`, **outside** `pol.budget_epoch` | 10,000 | 100,000 | 25% | 1 | TREASURY | [04](04-markets-and-pricing.md), [08](08-treasury-and-economics.md) §4.3 |
...
| `trs.proposer_reward` | Balance | NUM | PARAM 500; TREASURY/CODE min(0.05%·Ask, 25k); META 25k | ×0.1 | ×10 | ×2 | 2 | META | [08](08-treasury-and-economics.md) |
| `iss.inflation_cap` | Percent | /yr | 2% | — | amendable **down only** (K) | — | — | CONST | [08](08-treasury-and-economics.md) §2.3 |
| `fee.gov_num_rate` | Fixed | NUM/GOV | 1.0 × `fee.gov_num_rate_ref` (ref is K, set at genesis from launch price — **[VERIFY at TGE; placeholder ref 0.05 NUM/GOV]**) | **0.1 × ref (K)** | **10 × ref (K)** | ×2 | 1 | PARAM | [08](08-treasury-and-economics.md) §9 |
| `code.spacing` | u32 | blocks | 30 d | **14 d K floor** | — | — | 2 | META | [09](09-execution-upgrades-and-rollout.md) |
| `orc.bond_floor` (round-1 floor) | Balance | NUM | 10k | 2.5k | 100k | — | 2 | META | [07](07-oracle-and-disputes.md) §6 |
...
| `orc.reporter_stake` | Balance | NUM | 100k | 25k | 500k | ×2 | 2 | META | [07](07-oracle-and-disputes.md) |
| `dis.merit_min` (ProcessHold dispute-bond threshold) | Balance | NUM | **= `B_1(c, m)` (value-scaled per [07](07-oracle-and-disputes.md) §6.1; default equality)** | floor: `orc.bond_floor` | — | ×2 | 2 | META | [07](07-oracle-and-disputes.md) §12 |
| `wt.quorum` (watchtower ack) | u8 | of N registered | 2 | 2 (kernel floor) | 5 | 1 | 2 | META | [07](07-oracle-and-disputes.md) §4 |
| `wt.stake` (watchtower bond) | Balance | NUM | 25,000 | 10k | 100k | ×2 | 2 | META | [07](07-oracle-and-disputes.md) §4 |
...
| `reg.bond_incident` / `reg.bond_milestone` | Balance | NUM | 5,000 / 2,500 | ×0.5 | ×10 | ×2 | 2 | META | [07](07-oracle-and-disputes.md) §7 |
| `res.probe_interval` / `res.probe_timeout` | u32 | blocks | 14,400 / 600 | — | — | — | 1 | PARAM | [07](07-oracle-and-disputes.md) §8 |
| `res.probe_amount` | Balance | NUM | 0.10 (10 NUM-cents) | — | — | — | 1 | PARAM | [07](07-oracle-and-disputes.md) §8 |
| `res.fail_threshold` / `res.recover_threshold` | u8 | consecutive probes | 2 / 3 | — | — | — | 2 | META | [07](07-oracle-and-disputes.md) §8 |
| `grd.bond` / allowances | — | — | 50k GOV / [06](06-governance-and-guardians.md) table | — | scope K | — | — | entrenched | [06](06-governance-and-guardians.md) |
| `keeper.rebate` | Balance | NUM | ≈3× fee cost per sanctioned crank **[VERIFY fee basis at benchmark time]** | 1× | 10× | — | 1 | PARAM | [08](08-treasury-and-economics.md) §6 |
| `keeper.budget_epoch` | Balance | NUM | **12,000** (raised per keeper-medium; derivation [08](08-treasury-and-economics.md) §6.2) | 6,000 (floor covers decision-critical load — see §3 note) | 60,000 | ×2 | 1 | PARAM | [08](08-treasury-and-economics.md) |
| `collator.comp_epoch` | Balance | NUM/collator | **2,000** (frozen default, D-15) | 500 | 10,000 | ×2 | 1 | PARAM | [08](08-treasury-and-economics.md) §2.4 |
| `collator.n_min` (K component of `C_onchain`) | u8 | — | 4 | 3 | 12 | 1 | 2 | META | [05](05-welfare-and-decision-engine.md) §4.3 |
| `collator.bond_req_gov` (E-coverage requirement, per collator) | Balance | GOV | **[VERIFY — set with the collator program before Phase-3 arming; sim-gated]** | — | — | ×2 | 2 | META | [05](05-welfare-and-decision-engine.md) §4.3.1 |
...
| `ops.*` budget lines (`ops.bootnodes`, `ops.rpc_archive`, `ops.collators`, `ops.keepers`, `ops.oracle_evidence`, `ops.monitoring`, `ops.arweave`, `ops.watchtowers`, `ops.coretime`) | Balance | NUM/epoch | **[VERIFY — sized in Phase-2/3 ops planning; ops-gated]** (`ops.collators` = `collator.comp_epoch` × collator count; `ops.keepers` per [08](08-treasury-and-economics.md) §6.3) | — | — | — | 1 | TREASURY | [08](08-treasury-and-economics.md) §1.1, [12](12-release-and-operations.md) §6.1 |
| `grd.review_deadline` (guardian retro-ratification deadline) | u32 | epochs | 2 | 1 | 4 | 1 | 2 | META | [06](06-governance-and-guardians.md) §5.4 |
| `att.bond` (attestor bond) | Balance | GOV | 25,000 | ×0.5 | ×10 | ×2 | 2 | entrenched | [06](06-governance-and-guardians.md) §7 |
...
| `ledger.min_split` (= MinTransfer) | Balance | NUM | 0.01 (10⁴ base units) | 0.01 (K floor) | 1 | — | 2 | META | [03](03-conditional-ledger.md) |
| `ledger.archive_delay` | u32 | blocks | 1 yr | 90 d | — | — | 2 | META | [03](03-conditional-ledger.md) |
| `ledger.position_deposit` | Balance | NUM/entry | **0.1** (frozen, Part 3; raised from 0.01) | 0.1 (K floor) | 1 | — | 2 | META | [03](03-conditional-ledger.md) |
| `phase3.tvl_cap` (global real-NUM exposure, D-13) | Balance | NUM | 2,000,000 *(sim-gated **[VERIFY before Phase-3 arming]**)* | — | raised only by phase-gate META + values ratification | — | — | META+values | [09](09-execution-upgrades-and-rollout.md) |
| `phase3.deposit_cap` (per account, D-13) | Balance | NUM | 20,000 *(sim-gated **[VERIFY]**)* | — | as above | — | — | META+values | [09](09-execution-upgrades-and-rollout.md) |
...
| Constant | Value | Doc |
|---|---|---|
| `MinTrade` / `MaxTrade` | 1 NUM / b/4 per extrinsic (single-trade impact ≤ 0.25 logit) | [04](04-markets-and-pricing.md) |
...
| Keeper-budget floor note | the 6,000 NUM hard min of `keeper.budget_epoch` is a kernel floor: below it the metered budget cannot cover decision-critical cranks at the 1× rebate bound and A-1 fails silently | [08](08-treasury-and-economics.md) §6 |
...
- Maker worst-case loss `b·ln 2` per book; **worked maker-loss example ≈ 180 NUM** (TREASURY b = 25,000 walked 0.5 → 0.56; corrected from ≈ 1,507).

### 3.3 LMSR authoritative test vectors (b = 10,000 NUM, 64.64; regenerated from the reference model in CI — B-6)

| # | Action | Frozen value |
|---|---|---|
| V1 | cost of buying 1,000 LONG from q=(0,0) | `10000·ln((e^{0.1}+1)/2)` = **512.494795136…** NUM (**corrected** from 512.925464970) |
| V2 | price after V1 | 0.524979187479… |
| V3 | displace 0.5 → 0.6 | Δ = 4,054.65108108… LONG; cost = 2,231.43551314… NUM |
| V4 | worst-case loss | 6,931.47180560… NUM |
| V5 | V1 round trip net of 2 × 30 bps fees | net = **−3.074969…** NUM (**corrected** from −3.077552) |
...
- **Oracle adjudication track: 60% approval / 10% support / 7-day decision**, tally on a **pre-cohort conviction snapshot** (GOV locked before the subject cohort's creation) — raised from 50%/3%/5-day (B-19/D-18).
...
- Intake: ≤ 4 entries/epoch/account; 10% bond slash **to INSURANCE** (never burned — NUM is bridged USDC) on non-decision-grade and preimage-missing outcomes (§1 keys; economics [08](08-treasury-and-economics.md) §7).
- Keeper metered budget 12,000 NUM/epoch; collator compensation 2,000 NUM/collator/epoch ([08](08-treasury-and-economics.md)).
...
| NUM | `ForeignAssets`, Location `{parents: 1, X3(Parachain(1000), PalletInstance(50), GeneralIndex(1337))}` **[VERIFY asset index 1337]**; 6 decimals; `min_balance` 10⁴ |
| GOV | native; **total supply 10⁹, 12 decimals**; existential deposit 0.01 GOV |
| NUM treasury target | ≥ 25M NUM before Phase-5 arming; per-class min-viable NAV floors per [08](08-treasury-and-economics.md) §4 |
...
| Positions deposit | **0.1 NUM/entry** (`ledger.position_deposit`) | — | [03](03-conditional-ledger.md) |
...
3. **Positions:** globally priced by the 0.1 NUM deposit (dusting an account to its 64-cap now costs 6.4 NUM/victim-account, cf. threat row in [14](14-threat-model.md)); per-vault reap drains the `(PositionId, *)` prefix in `ReapBatch` = 100 chunks.
...
| B-low drift batch | §3.1 Trade = d5–d18 with fractional offsets; §3.1 maturity B+288,000; §3.2 κ per 10-block interval (ADR-11 corrected), maker-loss ≈ 180 NUM; §3.4 latency reconciliation pointer |
...
| B-med Positions map (owned by [03](03-conditional-ledger.md)) | §4: key order `(PositionId, AccountId)`, 0.1 NUM deposit, 64-cap + protocol exemption recorded as the single normative values |
| B-med keeper budget / D-15 / D-16 values | §1: `keeper.budget_epoch` 12,000 (floor 6,000 kernel-justified), `collator.comp_epoch` 2,000, `intake.*`, `fee.gov_num_rate`, `pol.b_baseline`, `orc.bond_bps`, `orc.window` 72 h, watchtower/attestor constants — all keys named with owning docs |
