...
2. **Rows whose default is not yet a number.** A row still tagged `[VERIFY]` or *sim-gated* under rule 4, or whose value is wholly derived at its consuming engine, carries no genesis record until adoption — a static record cannot represent a value that calibration has not yet fixed, or that is a live function of other state. Reading rule 2 binds such a row **from adoption onward**, at which point it is seeded like any other.
