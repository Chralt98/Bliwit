...
pub enum PositionKind {
    BranchNum,                 // branch-conditional NUM
...
- **Proposal instruments**: `2 branches × (1 BranchNum + 2 scalar + 2×2 gate) = 14` kinds per vault. Gate kinds exist for every vault but carry non-zero supply only for classes with gate books (CODE, META, TREASURY > 1% NAV).
- **Baseline instruments**: 2 kinds per epoch — LONG/SHORT on the epoch settlement score `s_e`, collateralized in NUM **directly, with no branch layer** (the Baseline market is unconditional).
...
pub struct BranchSupply {
    pub num: Balance,              // outstanding branch-NUM supply
...
pub trait Config: frame_system::Config {
    ...
    type Collateral: fungibles::Mutate<Self::AccountId, AssetId = AssetId, Balance = Balance>; // NUM
    ...
    type PositionDeposit: Get<Balance>;      // 0.1 NUM per Positions entry (normative value: §13)
...
|---|---|---|---|---|---|
| `split(pid, a)` | Signed | vault `Open`; `a ≥ MinSplit`; NUM transfer succeeds | `E += a`; mint `a` AcceptNum + `a` RejectNum to caller | `Split` | 1 asset transfer + 3 map writes |
| `merge(pid, a)` | Signed | vault `Open`, `Resolved`, or `Voided`; caller holds `a` of both branch-NUM | burn both; `E −= a`; transfer `a` NUM out | `Merged` | as split |
| `split_scalar(pid, b, a)` | Signed | vault `Open`; caller holds `a` `b`-NUM | burn `a` `b`-NUM; mint `a` LONG_b + `a` SHORT_b; `num_b −= a; Q_b += a` | `ScalarSplit` | 3–5 writes |
| `merge_scalar(pid, b, a)` | Signed | vault `Open`, `Resolved`, or `Voided`; caller holds `a` of both legs | inverse | `ScalarMerged` | — |
| `split_gate(pid, b, g, a)` | Signed | vault `Open`; caller holds `a` `b`-NUM | burn `a` `b`-NUM; mint `a` GateYes(g)_b + `a` GateNo(g)_b; `num_b −= a; G_{b,g} += a` | `GateSplit` | 3–5 writes |
...
Merge availability in `Resolved` and `Voided` is deliberate: **a complete Accept+Reject pair recovers par (1 NUM per pair) in every non-`ScalarSettled` state** — this is the D-1 primary recovery path. `merge_scalar`/`merge_gate` are value-neutral in every state (a complete set is worth exactly one branch-NUM under every valuation of §7.5) and stay available in `Voided` so set holders can climb back to branch-NUM and then to par. `transfer` stays available in `Voided` so counterparties can assemble pairs. In `ScalarSettled` the redemption calls (§5.3) subsume all of these.
...
|---|---|---|---|---|
| `redeem(pid, a)` | Signed | `ScalarSettled{winner: w, ..}`; caller holds `a` winning (`w`) branch-NUM | `a` (1:1) | `Redeemed` |
...
| `redeem_void(pid, kind_coords, a)` | Signed | vault `Voided`; caller holds `a` of the instrument | branch-NUM: `floor(a/2)`; LONG/SHORT/GateYes/GateNo: `floor(a/4)` (D-1; §7.5) | `VoidRedeemed` |
...
fn do_split(pid, who, a);            // buy leg: split cost c NUM → c AcceptNum + c RejectNum to buyer
fn do_transfer(position_id, from, to, a);   // pay target-branch c branch-NUM into the book;
                                            // the mirror-branch c branch-NUM REMAINS WITH THE BUYER (D-3)
...
Consequences the ledger guarantees (and the property tests assert): a wrapper buyer always ends the trade holding the mirror-branch branch-NUM equal to their paid cost, so under VOID the buyer's split is reconstructible to par (D-1 × D-3); book revenue never sits as bare branch-NUM across a block boundary — it is recycled into complete sets in the same extrinsic, so every LMSR obligation stays pre-collateralized in the ledger (I-12).
...
```
split(a):              E += a;  num_Acc += a;  num_Rej += a
merge(a):              E −= a;  num_Acc −= a;  num_Rej −= a          // payout a
split_scalar(b,a):     num_b −= a;  Q_b += a
merge_scalar(b,a):     num_b += a;  Q_b −= a
split_gate(b,g,a):     num_b −= a;  G_{b,g} += a
merge_gate(b,g,a):     num_b += a;  G_{b,g} −= a
resolve(w):            freeze branch ≠ w
void():                unfreeze all; enter Voided
settle_scalar(s):      record s;    settle_gate(g,o): record o
redeem(a):             E −= a;  num_w −= a                            // payout a
...
redeem_void(bNUM,a):   E −= floor(a/2);  burn a branch-NUM
...
```
E = num_b + Q_b + G_{b,S} + G_{b,C}        for EACH b ∈ {Accept, Reject}   (L-1)
```

equivalently `E = supply(AcceptNum) + Q_Acc + G_{Acc,S} + G_{Acc,C} = supply(RejectNum) + Q_Rej + G_{Rej,S} + G_{Rej,C}` — the BE §10.2 identity extended over the scalar **and gate** sets and stored per branch. Dual minting keeps the two right-hand sides equal; every intra-branch op moves value between `num_b`, `Q_b`, `G_{b,g}` of the *same branch* and each such op checks its own decremented field ≥ a. **There is no cross-branch counter to underflow.** For Baseline vaults: `E_base = sets = supply(B-LONG) = supply(B-SHORT)` while `Open`.
...
| Step | Op | E | num_Acc | Q_Acc | G_Acc,S | G_Acc,C | num_Rej | Q_Rej | G_Rej,S | G_Rej,C | L-1 (Acc / Rej) |
|---|---|---|---|---|---|---|---|---|---|---|---|
...
- `merge`, `merge_scalar`, `merge_gate` remain enabled: **complete pairs always recover par** (1 NUM per Accept+Reject pair; sets climb to branch-NUM first — value-neutral).
- `redeem_void`: unpaired branch-NUM pays `floor(a/2)`; unpaired LONG/SHORT/GateYes/GateNo pays `floor(a/4)`. Rounding against the redeemer; residue swept per R-5.

**Conservation argument.** Assign the VOID valuation `v(branch-NUM) = ½`, `v(any scalar or gate leg) = ¼`. Total claim value at entry to `Voided`:

```
V = Σ_b [ ½·num_b + ¼·(2·Q_b) + ¼·(2·G_{b,S}) + ¼·(2·G_{b,C}) ]
  = ½ · Σ_b [ num_b + Q_b + G_{b,S} + G_{b,C} ]
...
**The B-1 counterexample, re-run.** Split 100 NUM → 100 AcceptNum + 100 RejectNum; vault voided; `E = 100`:
...
- `Open`: `V = max_world payout = E` (by L-1: in world *b*, branch-NUM pays 1, a scalar set pays `s+(1−s)=1`, a gate set pays `1+0=1`, so world-b payout `= num_b + Q_b + ΣG_{b,g} = E`).
- `Resolved(w)`: only branch-w claims live; `V = num_w + Q_w + Σ_g G_{w,g} = E`. No outflow op except `merge` (pays 1 for cross-branch value 1+0 = 1, decrementing both sides of L-1).
- `ScalarSettled{w,s}`: `V = num_w·1 + [floor-bounded scalar claims ≤ Q_w-mass] + [gate winning side ≤ G-mass]`; each redemption op pays ≤ the exact value it burns (LONG `floor(a·s) ≤ a·s`, SHORT `floor(a·(1−s)) ≤ a·(1−s)`, pair exactly `a`, gate winning side `a`, losing side 0).
...
- **R-2.** `MinSplit = MinTransfer = ledger.min_split = 0.01 NUM (10^4 base units)` *(normative value: [`13-parameters.md`](./13-parameters.md))*; positions cannot be created below it; transfers leaving a remainder below it MUST move the whole balance.
- **R-3.** Fees (30 bps) are charged by `pallet-market` per the trade-path rule of [`04-markets-and-pricing.md`](./04-markets-and-pricing.md) §6.1: on `buy` the fee is collected as a **complete branch-NUM pair** (both legs to the market's `fees_accrued` account — worth exactly the fee in NUM at any settlement); on `sell` it is withheld **single-sided** in target-branch branch-NUM. To the ledger these are ordinary, fully collateralized positions created by the same split/transfer ops as any other — no special fee path exists, so fee handling cannot break conservation. Realized fee value routes 50% INSURANCE / 50% POL-offset at settlement (economics: doc 08). Fees are non-refundable on all paths, including VOID (buy-side fee pairs merge at par to the protocol; sell-side withheld legs follow their branch).
- **R-4.** NUM is a sufficient asset, `min_balance = 10^4`; ledger sovereign, treasury sub-accounts and book accounts are genesis-endowed and can never be reaped; redemptions below `min_balance` are routed to the caller's existing balance or rejected with `BelowMinimum`.
...
- **R-6.** NUM sent directly to pallet accounts outside protocol flows is recoverable only by TREASURY-class proposal (`recover_foreign`), never by any admin.
...
| ID | Invariant |
|---|---|
| L-1 | Per-branch conservation: ∀ vault in `Open`/`Resolved`/`Voided`-pre-redemption bookkeeping: `escrowed == num_b + Q_b + G_{b,S} + G_{b,C}` for **both** branches; Baseline: `E_base == sets` while `Open`. Supply fields ≡ `PositionTotals` ≡ Σ `Positions` per instrument. |
...
| L-3 | Terminal valuation bound (integer forms): `ScalarSettled`: `E·10^9 ≥ supply(num_w)·10^9 + supply(L_w)·s + supply(S_w)·(10^9−s) + Σ_g supply(winning gate leg g)·10^9`. `Voided`: `4·E ≥ Σ_b [ 2·num_b + supply(L_b) + supply(S_b) + Σ_g (supply(Yes_{b,g}) + supply(No_{b,g})) ]`. |
...
|---|---|
| **B-1** (VOID pays 2× escrow) | §6.4: `Voided` pays pairs par via merge, unpaired branch-NUM `floor(a/2)`, legs `floor(a/4)`; valuation argument shows total payout ≤ E on every path; the 100-NUM counterexample is a named regression vector; PT-2 restated honestly. |
...
| **B-3** (Baseline market has no ledger home) — ledger side | §2.1/§2.2/§5: `BaselineVaults: map EpochIndex → BaselineVaultInfo`, `PositionId::Baseline{epoch, Long/Short}`, NUM-direct collateral, `split/merge/settle/redeem_baseline(_pair)`, settlement via SettleAuthority at e+3; `pol.b_baseline` referenced (value: doc 13; market side: doc 04). |
...
| **Positions map** (medium: unbounded, unpriceable, un-reapable) | §4: key order `(PositionId, AccountId)` — per-vault reap drains prefixes; per-account bound via `PositionCount` counter map; storage deposit 0.1 NUM/entry; protocol accounts exempt from `MaxPositionsPerAccount = 64`. |
