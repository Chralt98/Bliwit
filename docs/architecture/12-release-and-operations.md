...
| **Reserve-health probe** | Keep the dedicated USDC accounting line funded through [08](08-treasury-and-economics.md) §1.1's runway-capped bootstrap/governed path; separately maintain the probe USDC and pre-position/replenish the actual DOT fee inventory in Bleavit's sovereign account on Asset Hub; publish the live barrier/fee/response and bidirectional-HRMP evidence required by [07](07-oracle-and-disputes.md) §8, including ≥ 1 timely authenticated pass. Before arming and at every ops funding review, verify that the local line covers `res.fail_threshold + res.recover_threshold` full debits (5 at genesis), remote USDC covers the probe amount, and remote DOT covers the same number of envelopes plus a refill margin. Complete the one-way handover with a successful positive TREASURY-class reserve-line funding and verify the bootstrap latch closed before Phase-4 sudo removal. Missing calibration, route evidence, pass evidence, funding leg or handover is release-invalid rather than a healthy probe state | Oracle operations coordinator | `ops.reserve_probe` |
...
- **Reserve-probe runway (XCM-domain extension).** The XCM domain MUST additionally expose the local `ops.reserve_probe` balance as full-envelope runway and independently observe the Asset Hub sovereign account's USDC and DOT balances. It alerts through RB-XCM before the local line falls below `res.fail_threshold + res.recover_threshold` full debits, before remote USDC falls below `res.probe_amount`, or before remote DOT falls below that same number of full `ops.probe_fee_dot` envelopes **plus the operations refill margin**. The parameterized thresholds are evaluated from live Params and the remote balances are never inferred from the local accounting line. This is one XCM-domain funding-readiness signal, not a twenty-second monitoring domain; its implementation must extend the existing XCM rule and inventory atomically so the §6.3 coverage gate remains exact.
...
- **"Cursor stalled" (Upgrades row)** *(added 2026-07-20, SQ-265; reconciled 2026-07-22, SQ-132)* is the **union** of three conditions, any one of which raises the alert: the latched stall halt, the live elapsed-budget predicate over an active migration cursor, and a cursor in the `Stuck` state. A permanently failed migration is strictly more alarming than a slow one and RB-UPGRADE (PB-MIGRATION) owns both dispositions, so they share one signal rather than competing for an operator's attention. The union is deliberately **not** "any migration halt is latched" — halt sources other than the stall latch carry their own dispositions and are not folded in here. "Stalled" is exactly the [09 §3.2(d)](09-execution-upgrades-and-rollout.md) time budget, `now − cursor.started_at > MIGRATION_STALL_BLOCKS`, never cursor-byte immobility; the detector and runbook read the SDK cursor's own `started_at`, so a lawful migration returning byte-identical cursors does not false-raise.
