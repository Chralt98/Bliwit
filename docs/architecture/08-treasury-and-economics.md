...
| `INSURANCE` | Slash proceeds, swept dust, fee share | inflow-only except by TREASURY decision — the one admissible outflow is `sweep_insurance` to `MAIN` (§1.2) |
...
- **NAV effect.** The swept amount leaves the excluded INSURANCE balance and enters NAV. This is why the sweep must be a TREASURY *decision* rather than an administrative transfer: it is the only operation that can raise NAV without an external inflow, and NAV is the divisor of the §4.1 arming floors and the in-cap prize. Routing it through the market makes that raise a decided act with a `DecisionRecord`, not a bookkeeping move.

A dedicated `BudgetLine::Insurance` was rejected: budget lines are *funded from* `MAIN` and spent outward, so modelling an inflow-fed reserve as a line inverts the direction of every §1.1 control. Declaring INSURANCE a permanent sink was also rejected — it would contradict §1.1's own text and strand bridged Asset-Hub USDC forever, which is precisely the outcome §7.1's anti-burn rationale exists to avoid. This settles the accounting question **and** the reachability question; the implementation of `sweep_insurance` is tracked separately (SQ-207).
...
- Requirement 3P = 2,079,441 ≤ 2,332,616 ✔ — holds with margin 253,175 USDC (= 1.5 × 168,783 = **12.2%**, the same margin ratio (b) records, since `b_floor·ln2 / P_ref` is class-invariant across the defaults). Cap check: P = 693,147 ≤ AttackCost̂/3 = 777,539 ✔. The `sec.flow_cap` ceiling at its ×7 minimum is 7 × 243,502 = 1,704,514 ≥ 1,386,294 — not binding, per §5.3's non-bindingness bound.
- **Floor-depth lower bound (conservative illustration, not the normative seeding).** At the unscaled floor `b = 60,000`, POL depth = 83,178, L̂ = 1,469,472 and AttackCost̂ = 2,204,208, still clearing 3P = 2,079,441 with margin 6.0%. This arithmetic is retained because the identity clears even there — but §5.3 is explicit that a book seeded at floor `b` while `v_min` carries the `2P` scaling **is not a configuration the normative seeding produces**, so it must not be read as the worked example. The superseded (a) printed exactly that unscaled figure as the normative result, contradicting §5.3 (SQ-301, 2026-07-21); (b) already carried its scaled companion, making (a) the unfixed twin.
