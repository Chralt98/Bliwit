...
**`updated_at` (offset 108) is written by every writer (normative; SQ-133 resolution, 2026-07-20).** The field records the block of the **last write to the record**, not the last manifest repoint: writer (a) sets it on each of its three occasions (authorization, applied-upgrade detection, and the §2.1(7) abort cleanup) exactly as writer (b) does on a repoint. The per-writer parentheticals in the writers sentence above — and in [02](./02-integration-contract.md) §12 — enumerate the fields each writer sets *purposefully*; they give neither writer exclusive title to an offset, and `updated_at` is metadata about the record itself. Manifest freshness is carried by `manifest_txid`/`release_json_hash`, which writer (a) never touches; monitoring that needs repoint lag MUST derive it from the manifest pointer, never from offset 108.
...
- **Rollback** otherwise (logic fault, storage-shape mismatch, second failed retry): rollback is a **forward upgrade** carrying a remediation artifact whose migration restores the checkpoint semantics for the affected keys. It rides the **expedited CODE lane** (§3.1, admissible because the halt is active) with full attestation and fast-track ratification. Guardians cannot install code (kernel prohibition, carried); their playbook power ends at freezing (the automatic `MigrationHalt` bridge) and *initiating* the expedited-CODE lane — the in-framework retry controls are Root-only, not guardian-dispatchable (SQ-274).
...
- `phase3.deposit_cap` — per-account cumulative USDC deposit cap over the phase; enforced at the same points. At the XCM leg the beneficiary is knowable only at the **deposit step** (after the mint), so a per-account rejection traps the already-minted holding — recoverable by the sender via the §6.1 self-scoped `claim_assets` row (SQ-110). The cumulative per-account meter is on-chain state shared by every enforcement point (`pallet-inflow-caps`; the XCM leg records through it transactionally, so an aborted message records nothing).
