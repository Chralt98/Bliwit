...
G-3. **Annulment.** On **normal losing-branch resolution**, trades in the unrealized branch are economically reverted for the dominant user path: buyers hold the mirror branch-USDC (D-3) and redeem `cost` at par, losing only fees. Complete-set holders always recover par, including under VOID (D-1). Under **protocol VOID** a wrapper buyer's whole package instead recovers its D-1 **neutral value** after pair-first netting — equal to the debit only at the neutral prior 0.5 — and a deliberately unpaired holder receives that same neutral valuation. No claim is ever haircut below the D-1 schedule, but VOID does not refund a premium paid over the neutral prior (SQ-171, 2026-07-21).
