...
**Cohort-VOID proposal disposition (normative; reconciled with §2.1 — SQ-314).** The cohort-VOID path partitions the epoch's affected proposals by whether a decision has already been recorded, and treats the two populations differently:

- **Already-decided proposals** — the cohort's own members, which by construction have reached `Measuring` or beyond — MUST retain their recorded state and their recorded `DecisionOutcome` unchanged, and MUST NOT emit `ProposalForceRejected`. They take the cohort/vault treatment only: any market-backed vault moves to `Voided` ([doc 03](./03-conditional-ledger.md) §2.3) and the cohort carries the `CohortVoided` outcome. They are archived with the decision the market actually produced.
- **Same-epoch proposals that have not reached a decision** take T20: the path MUST terminally archive them with state `Rejected(ProcessHold)` and `DecisionOutcome::Reject(ProcessHold)`, MUST emit `ProposalForceRejected`, and MUST move any market-backed vault to `Voided`.

Both populations leave the non-terminal working set in the same transaction. The split is mandatory rather than cosmetic: the cohort archive is the only durable record of what the market concluded, so overwriting a completed `Adopt` with `Reject(ProcessHold)` would record a rejection the market never produced ([doc 07](./07-oracle-and-disputes.md) §10: "decisions already made stand"). An earlier revision of this paragraph required the overwrite for *every* affected proposal, contradicting §2.1's normative note; §2.1 and doc 07 govern, and this paragraph is now stated to agree with them.
