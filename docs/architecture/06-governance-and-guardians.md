...
**Guard-record reaping ownership (normative; SQ-192 resolution, 2026-07-20).** The execution guard's per-proposal records — the queue entry and its pinned preimage, `Ratifications`, the expedited marker, retained rerun pins, and attestation bindings — are reaped by **one owner: the epoch pallet's terminal callback**, invoked whenever a proposal reaches any terminal state. That ownership is deliberately *universal* rather than queue-scoped, so it also covers the two cases the queue-scoped reading leaves stranded: a proposal that was **ratified but never queued** (rejected or expired before reaching the queue, whose `RatificationRecord` would otherwise outlive it), and a **rerun that was retained but not re-queued**. The callback MUST be **idempotent** — it is legitimately reached more than once (the guard also self-cleans on its own terminal paths, and terminal callbacks can double at seam boundaries), so a second invocation is a no-op returning `Ok`, consistent with §2.1's idempotency rule. Reaping never depends on the proposal having been queued.
...
Every activation **that dispatches an effect** emits `PlaybookActivated { id, trigger, expiry }`, consumes the per-playbook allowance, writes a review record, and schedules mandatory retrospective ratification (§5.4). Expiry emits `PlaybookExpired { id }` and reverts the playbook's effects automatically.
...
- **Expiry.** Auto-expires after ≤ **14 days** (201,600 blocks; kernel constant, *(normative value: [13](./13-parameters.md))*). **One renewal only**, for ≤ 14 further days, via a values referendum on the `guardian` track (`guardian.renew_playbook(PB_LEDGER_FREEZE)` — 7-day decision fits inside the first expiry window). **The renewal installs one fresh kernel window measured from the renewal block, not from the original expiry** (normative; SQ-45(b) resolution, 2026-07-20): total freeze duration is therefore ≤ 28 days, and strictly less whenever the renewal enacts before the first window closes. No second renewal exists; a longer freeze means the repair has failed and VOID/recovery paths ([03](./03-conditional-ledger.md), [09](./09-execution-upgrades-and-rollout.md)) take over.
...
- **Challenge window.** Each attestation is challengeable for **72 h** (43,200 blocks) from submission by anyone posting a bond ≥ 50% of **the bond held by the challenged attestor at the time the challenge is opened** — not the live `att.bond` registry value, which may have been amended since that attestor was seated (normative; SQ-248 resolution, 2026-07-20): `attestor.challenge_attestation(attestation_id, evidence_hash)`. A challenged attestation does not count toward quorum while the challenge is open. Resolution: by deterministic recomputation proof where the dispute is reproducibility (any keeper submits it, mechanically resolving), else by a `ratify`-track adjudication referendum. The losing side forfeits 50% of its bond on the **same held-bond basis** — the challenger's forfeit is 50% of its posted bond, the attestor's 50% of its held bond at resolution, both rounded **up**, against the forfeiting party (attestor additionally ejected on a second adjudicated-false attestation). An amendment to `att.bond` ([13](./13-parameters.md)) therefore binds a member only when that member is next seated by `attestor.set_members`; the stored basis is the only reading that keeps the challenger's stake proportionate to the attestor's real collateral under amendments in **both** directions (a raise would otherwise price contesting out, a cut would otherwise make challenge-spam denial of attestation cheap). Dispute-game plumbing follows the oracle pattern of [07](./07-oracle-and-disputes.md).
