...
1. **Differential corpus.** The ≥ 10⁷-point MPFR corpus is exported as JSON from `reference-model/` and published with each release as a backend test artifact (D-2/X-15; feed frozen in [02-integration-contract.md](./02-integration-contract.md)).
...
- Accumulator `A += o_t·Δblocks` in u256 (two-limb); `window_checkpoints: BoundedVec<(BlockNumber, Cum), 8>` at decision-window and trailing-window boundaries give O(1) `TWAP(w) = (A(end) − A(start))/blocks`.
