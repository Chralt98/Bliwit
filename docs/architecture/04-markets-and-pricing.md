...
- **Protocol VOID (D-1).** Under `VaultState::Voided`, `merge`/`merge_scalar` stay enabled and every instrument redeems at its neutral-prior value (unpaired branch-USDC `floor(a/2)`, unpaired LONG/SHORT `floor(a/4)`; pairs always 100% — normative rules in [03-conditional-ledger.md](./03-conditional-ledger.md)). A buyer's package is `amount` target scalar units plus `cost` mirror bUSDC. Exact pairs merge at par first; only residual claims are floored. For an isolated buy with no pairable offsets, recovery is `floor(amount/4) + floor(cost/2)`, so net delta against the `cost + fee` debit is `recovery − cost − fee` — **`−fee` only at the neutral prior `p = 0.5`, where `cost = amount/2`** (SQ-171, 2026-07-21; the superseded "recovery is par for buyers on the protocol path" wording silently assumed that prior). What D-1 guarantees is that **no instrument is valued below the neutral-prior schedule** — not that a premium paid over it is refunded. A buyer who paid above the neutral prior bears that premium; one who bought below it gains. This is the same footing as the deliberately unpaired speculator, which is the point: VOID prices every claim neutrally rather than picking winners.
...
- **`BaselineMarketOf: map EpochIndex → MarketId`** — declared in `pallet-market` storage, written at Baseline book creation (Seed, epoch e). The mapping **outlives the Baseline book**: it MUST be retained while epoch `e` is present in the `RecentCohortSummaries` ring or is a live epoch, and is pruned in lockstep with ring eviction (≤ 36 entries) — reaping the Baseline market MUST NOT remove it. The superseded "reaped with the market" wording was drift: `ledger.archive_delay` (~1 yr) elapses roughly ten months *before* the 32-cohort ring evicts, so reap-time deletion would leave the frozen contract advertising a mapping the chain had already dropped. Shape, name and retention rule are **frozen in [02-integration-contract.md](./02-integration-contract.md) §7.4**; the decision engine's `baseline_market(epoch)` accessor and the frontend's Baseline reads both resolve through it (X-10 closure). The frontend precondition row for Baseline trading exists in doc 11.
- **Semantics of a retained id whose book has been reaped (normative; SQ-304, 2026-07-21).** Because the mapping outlives the book, `BaselineMarketOf[e]` MAY resolve to a `MarketId` for which no `MarketState` exists. That id is then **historical correlation data only** — it names which book *was* the epoch's Baseline, and nothing more. Consumers MUST treat the absent book as absent rather than as a book quoting zero: the decision engine's accessor and every runtime view MUST fail closed (no quote, no depth, `within_domain = false`), and the frontend MUST label the book reaped, MUST NOT render a zero quote as a market price, and MUST disable every trade action on it (precondition rows in doc 11). Retaining a bounded `MarketState` tombstone was rejected — it would grow always-served state and re-open reap semantics for no consumer benefit, since cohort history is already served by `RecentCohortSummaries`.
