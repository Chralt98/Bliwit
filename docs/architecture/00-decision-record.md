...
- **Amended 2026-07-19 (SQ-271 at-risk refinement):** per-book contest capital removes `min(q_LONG−q_pol_LONG, q_SHORT−q_pol_SHORT)` before marking, so balanced complete-set inventory that pays par for every settlement cannot certify depth. Only the unmatched directional leg remains, marked at its side's previous-block quote and rounded down. A seeded PARAM probe removed all six profitable TH-6 belief-capture flips through the 3×3P search ceiling: the attacker's real directional leg still counts, but the approximately 75k/book balanced holder inventory no longer self-certifies it. The SQ-272 decision-pair **MIN** reduction is unchanged. User-authorized by the staged GO/NO-GO task; owning text: 04 §7a, 05 §5.2/§5.6, 08 §5.2–§5.3.
