...
- New call `redeem_void(pid, kind, amount)`: unpaired **branch-NUM pays `floor(a/2)`**; unpaired **LONG or SHORT pays `floor(a/4)`** (equivalent to branch value 0.5 × neutral s = 0.5). Rounding is against the redeemer; residue swept per the dust rule.
- Conservation: total payout ≤ E in every path (pairs pay 1 per pair; each pair = 1 NUM escrowed).
- PT-2 (annulment) restated honestly: *complete-set holders and market buyers recover full principal under VOID* (buyers hold the mirror branch-NUM per D-3, so their split is reconstructible); a deliberately unpaired single-branch speculator recovers 0.5 — this is the correct price of a voided binary claim, not a loss of principal on the protocol path.
...
### D-3. Trade denomination: branch-NUM books with an auto-split wrapper; buyers keep the mirror (B-7)

- LMSR books are denominated in **branch-NUM**. The user-facing `buy(market, side, amount, max_cost)` accepts **NUM**: the wrapper splits cost `c` NUM into `c` AcceptNum + `c` RejectNum, pays the target-branch `c` into the book, delivers `q` LONG/SHORT of the target branch, and **credits the mirror-branch `c` branch-NUM to the buyer**. `sell` is the inverse.
- Consequence: a buyer in the losing branch holds mirror branch-NUM redeemable at par → G-3 (annulment) holds for the dominant user path, and VOID recovery (D-1) is par for buyers.
- **Revenue recycling**: book revenue (branch-NUM) is immediately `split_scalar` into complete LONG+SHORT sets held by the book — worth exactly 1 branch-NUM per pair at any settlement `s`, so the book is solvent by construction. `headroom = b·ln 2` per book, stated and sized in `04-markets-and-pricing.md`.
...
### D-12. NUM funding is IN SCOPE for the canonical frontend (X-8, X-14)
...
- **Fees (X-14)**: `pallet-asset-tx-payment` is wired to a constitution key **`fee.gov_num_rate`** (typed, bounded [0.1×, 10×] around its reference, PARAM-adjustable, genesis default set from the launch reference price). NUM-only users can always pay fees in NUM. The FE fee-currency selector binds to this key.
...
- Phase 3 runs under a **real-NUM exposure cap**: global TVL cap + per-account deposit cap (constitution keys, raised only by phase gates).
...
- **GOV**: total supply 1,000,000,000 (12 decimals). Allocation: 30% treasury reserve; 25% community distribution (vested); 20% founding team (4-year vest, 1-year cliff); 15% ecosystem/ops fund; 10% Phase 3–4 incentive programs. `iss.inflation_cap = 2%/yr`, issuance mechanism specified in `08`.
- **NUM treasury**: initial funding target **≥ 25M NUM** before Phase 5 arming; published **minimum-viable NAV per class** gates phase advancement (one CODE ⇒ NAV ≥ ~14M at floor liquidity; the gate is explicit and loud, not silent).
- **Collator compensation**: treasury ops line, 2,000 NUM/collator/epoch initial (PARAM-adjustable).
...
- Named treasury budget lines: bootnodes/RPC/archive (the 30-day commitment of D-6), keeper subsidies beyond the metered budget (raised to **12,000 NUM/epoch**, recomputed from crank volume), oracle evidence hosting, monitoring, Arweave/ArNS.
...
| NUM | `ForeignAssets`, XCM `Location {parents: 1, X3(Parachain(1000), PalletInstance(50), GeneralIndex(1337))}` **[VERIFY asset index 1337]** — pinned in FE `ChainIdentity` |
| NUM decimals | 6 · GOV decimals | 12 |
| GOV existential deposit | 0.01 GOV |
...
- **Oracle adjudication track (B-19)**: raised to 60% approval / 10% support / 7-day; tally uses a **pre-cohort conviction snapshot** (GOV locked before cohort creation), excluding capital that entered after the dispute's subject cohort began.
...
- **GOV reflexivity (B-10)**: the E component values security collateral as dimensionless **coverage ratios** against GOV-denominated requirements; no GOV price enters W anywhere.
...
| B-4 | Per-branch supply fields; per-branch identity `escrowed == supply(bNUM_b) + Q_b` checked for both branches; POL seeding flow re-walked | 03 |
...
| B-med: Positions map | Key order `(PositionId, AccountId)` (per-vault drainable); per-account bound via counter; deposit 0.1 NUM/entry; protocol accounts exempt from the 64-position cap | 03 |
...
| B-med: keeper budget | Recomputed (≥134k decision-critical cranks/epoch); metered budget 12,000 NUM/epoch + fee rebates; exhaustion alarm | 08 |
...
| B-low (all) | `ScalarSettled` carries winning branch; maker loss ~180 NUM; maturity B+288,000; Trade phase d5–d18 labels; TWAP slew per 10-block observation interval; §15.2 latency table reconciled with the D-18 track; dangling refs removed; FGP/SGF/GFP/EFP/AEGIS identified in the bibliography | owning docs |
| F-1 | D-6 | 10 |
...
- `IntakeQueue = 64`; `MaxLiveProposals = 32`; books/proposal ≤ 6; `MaxLiveMarkets = 196`; `RecentCohortSummaries` ring = 32; `MaxPositionsPerAccount = 64` (protocol accounts exempt); Positions deposit 0.1 NUM; key order `(PositionId, AccountId)`.
- LMSR: V1 = **512.494795136** NUM; V5 net = **−3.074969**; V2–V4 unchanged; maker worst-case loss `b·ln 2`; §11.10 maker-loss example ≈ 180 NUM; TWAP slew cap per **10-block** observation interval.
- VOID: pairs par via merge; unpaired branch-NUM `floor(a/2)`; unpaired LONG/SHORT `floor(a/4)`.
...
- Keeper metered budget 12,000 NUM/epoch. Collator comp 2,000 NUM/collator/epoch. Intake: ≤4 entries/epoch/account; 10% bond slash (to INSURANCE — burning NUM would strand backing reserve) on non-decision-grade/preimage-missing outcomes.
- Chain identity per D-17. GOV supply 10⁹ (12 dec); NUM 6 dec; treasury target ≥ 25M NUM; min-viable NAV per class per 08.
