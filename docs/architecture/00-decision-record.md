...
- PT-2 (annulment) restated honestly, **corrected 2026-07-21 (SQ-171)**: *complete-set holders recover par under VOID.* A D-3 wrapper buyer's package — the purchased target scalar leg plus `cost` mirror branch-USDC — recovers its **D-1 neutral value** after pair-first netting, which equals the original debit only at the neutral prior 0.5; net delta is `neutral recovery − cost − fees`, not `−fees`. A deliberately unpaired single-branch holder receives that same neutral valuation (0.5 per branch-USDC, 0.25 per leg). This is the correct price of a voided binary claim, not a loss of principal — but the earlier wording *"market buyers recover full principal under VOID"* over-claimed, because it silently assumed the neutral prior. What D-1 guarantees is that no claim is valued **below** this schedule, not that a market premium is refunded.
